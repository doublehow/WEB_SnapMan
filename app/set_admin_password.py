...
用法（專案根目錄）：
  python -m app.set_admin_password            # 互動輸入兩次（不回顯），寫入 config.json
  python -m app.set_admin_password --disable  # 清空 = 停用本機登入（須已啟用 AD，否則拒絕）

刻意不接受參數 / 管線輸入，避免密碼留在 shell 歷史或排程腳本。
服務執行中亦可使用：save_settings 為原子寫入，但執行中的程序仍持有舊值，重啟後生效。
"""
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="設定 SnapMan 本機 admin 密碼")
    ...
    print("已更新本機 admin 密碼（config.json）；服務若在執行中請重啟以套用。")
    return 0
