...
_SECURITY_HEADERS = {
    ...
    # 以下三個為弱掃常見 Info/Low 項：不需要任何裝置權限、隔離跨來源視窗、
    # X-XSS-Protection 已淘汰但部分掃描器仍檢查（設 0 = 明確停用舊式過濾器）
...
if __name__ == "__main__":
    ...
    # server_header=False：不送 Server: uvicorn（弱掃 banner disclosure 項）
    uvicorn.run("app.main:app", host=settings.web_host, port=settings.web_port,
                log_config=LOG_CONFIG, server_header=False)
